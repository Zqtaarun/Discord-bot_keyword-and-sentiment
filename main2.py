...
async def hello(ctx): # context
    ...
    await ctx.send(f'13. Saya bisa membantumu menulis(overwrite), baca, tambahkan(append) $tulis 2 ayo bangun; $baca; $tambahkan hello world')
    ...
    await ctx.send(f'16. Saya bisa membantumu mendapatkan sentiment dari sebuah kalimat dengan ketik $sentiment <kalimat> contoh $sentiment I love you')
    await ctx.send(f'17. Waktu sekarang $waktu')
    await ctx.send(f'18. Simpan Lokal, upload file lalu tuliskan $simpan')
    await ctx.send(f'19. klasifikasi pipit/merpati/jenis daun upload file lalu $klasifikasi atau $daun')
    await ctx.send(f'20. deteksi object upload file lalu $deteksi')    
    await ctx.send(f'21. Bantuan/saran ketik $help') 
    await ctx.send(60*'=')
